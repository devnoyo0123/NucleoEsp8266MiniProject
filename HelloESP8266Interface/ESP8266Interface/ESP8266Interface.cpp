...
int ESP8266Interface::id_send(int id, const void *data, unsigned size)
{
    _esp.setTimeout(ESP8266_SEND_TIMEOUT);
 
    if (!_esp.send(id, data, size)) {
        return NSAPI_ERROR_DEVICE_ERROR;
    }
...
int ESP8266Interface::id_recv(int* id, void *data, unsigned size)
{
    _esp.setTimeout(ESP8266_RECV_TIMEOUT);
 
    int32_t recv = _esp.recv(id, data, size);
    if (recv < 0) {
        return NSAPI_ERROR_WOULD_BLOCK;
    }
...
bool ESP8266Interface::readable()
{
    return _esp.readable();
}

bool ESP8266Interface::writeable()
{
    return _esp.writeable();
}

bool ESP8266Interface::tcpServerStart(int port){
    return _esp.tcpServerStart(port);
}